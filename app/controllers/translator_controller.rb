class TranslatorController < ApplicationController
  ...
  protected
    ...
    def register_translator_view
      register_view :translator_view, view_name_for(@file.file_id)
    end

    def register_markup_translator_file
      register_model @file, model_name_for(@file.file_id)
    end

    def model_for(file_id)
      send(model_name_for(file_id))
    end

    def model_name_for(file_id)
      "model_for_#{file_id}"
    end

    def view_for(file_id)
      send(view_name_for(file_id))
    end

    def view_name_for(file_id)
      "view_for_#{file_id}"
    end

    def connect_translator_contents_changed_signal_for(file_id)
      @signal_handlers[file_id] = view_for(file_id).textview.buffer.signal_connect('changed') do |textbuffer, iter, text, length|
        update_browser_preview(file_id)
      end
    end

    def disconnect_translator_contents_changed_signal_for(file_id)
      view_for(file_id).textview.buffer.signal_handler_disconnect(@signal_handlers[file_id])
    end

    def update_browser_preview(file_id)
      self.manager.save_temp_markup_file(file_id, view_for(file_id).textview.buffer.text)
      display_browser_preview(file_id)
    end

    def add_file_page_for(file, options = {})
      main_controller.main_view.add_file_page(file, view_for(file.file_id).root_widget, options)
    end
    ...
    def display_browser_preview(file_id)
      ...
      view_for(file_id).display_browser_preview(path)
    end
    ...
    def update_file_page_contents_for(file)
      view_for(file.file_id).textview.buffer.text = file.contents || ""
